...
from models.feature_extractor import feature_extraction
from models.aggregation import CoeffsPredictor
from models.refinement import HourglassRefinement
...
class GuideNN(SubModule):
    ...
    def __init__(self, params=None):
        ...
        self.params = params
        self.conv1 = binary_convbn_relu(32, 64, 3, 1, 1)
        self.conv2 = binary_convbn(64, 1, 3, 1, 1)
        self.conv3 = nn.Tanh()
...
class PBCStereo(SubModule):

    def __init__(self, groups):
        super(PBCStereo, self).__init__()
        self.groups = groups
        ...
        self.dres0 = nn.Sequential(binary_convbn_3d_relu(groups, 32, 3, 1, 1), binary_convbn_3d_relu(32, 16, 3, 1, 1))
        self.guide = GuideNN()
        self.slice = Slice()
        self.weight_init()
    ...
    def forward(self, left_input, right_input):
        ...
        cost_volume = build_gwc_volume(left_gwc_feature, right_gwc_feature, 25, self.groups)
        ...
        left_half = F.interpolate(left_input, scale_factor=1 / pow(2, 1), mode='bilinear', align_corners=False, recompute_scale_factor=True)
        right_half = F.interpolate(right_input, scale_factor=1 / pow(2, 1), mode='bilinear', align_corners=False, recompute_scale_factor=True)
        refinement_disp = self.refinement_net(half_disp, left_half, right_half)
        out1 = F.interpolate(refinement_disp * 2.0, scale_factor=(2.0, 2.0), mode='bilinear', align_corners=False,
